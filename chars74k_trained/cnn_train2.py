...
batch_size = 200	# How many samples per batch use
learning_rate = 0.001

# Load NUMBERS data into 

X_train = np.expand_dims(np.asarray(printed.images, dtype=np.float32),axis=1)
X_test = np.expand_dims(np.asarray(printed.test_images, dtype=np.float32),axis=1)
Y_train = np.asarray(printed.features, dtype=np.int64)
Y_test = np.asarray(printed.test_features, dtype=np.int64)
